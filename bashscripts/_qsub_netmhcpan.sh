...
# Thank you chatgpt for writing this wrapper lol
...
# Replace 'FILENAME' in the script with the actual filename provided as an argument
script=${script//FILENAME/$1}

# Write the modified script to a new file
echo "$script" > "${1}_qsub.sh"
chmod +x "${1}_qsub.sh"

echo "New script created: ${1}_qsub.sh"

qsub -W group_list=vaccine -A vaccine -l nodes=1:ppn=40:thinnode,mem=80gb,walltime=${walltime} "${1}_qsub.sh"
rm "${1}_qsub.sh"
